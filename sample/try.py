...
while True:
    ...
    if scene == 1:
        ...
        for e_laser_i, e_laser in enumerate(e_lasers):
            ...
            # 急に簡単になる,自分のレーザーで相手のレーザーを消せる仕様

            for p_laser_i, p_laser in enumerate(p_lasers):
                p_laser_rect = pygame.Rect(p_laser_surface.get_rect())
                p_laser_rect.left, p_laser_rect.top = p_laser
                if e_laser_rect.colliderect(p_laser_rect):
                    e_lasers.pop(e_laser_i)
                    p_lasers.pop(p_laser_i)
                    #k-tuika
                    p_hit_e_laser_counter += 1
                    p_laser_hit_sound.play()
                    # k-tuika
                    is_shot = False
        ...
        hit_text = font_counter.render(
            "hit : " + str(p_hit_enemy_counter + p_hit_e_laser_counter), False, (255, 255, 255)
        )
    ...
    if scene == 2:
        ...
        # 8.2 - 命中率を表示
        # k-tuika
        hit_rate = (p_hit_enemy_counter + p_hit_e_laser_counter) / p_laser_counter
    ...
    # # 11 - player の動作
    # if enemies or e_lasers:
    #     p_x = enemies[0][0] 
    #     if len(enemies) > 1 and len(e_lasers) > 1:
    #         min_enemie = enemies[0][1]
    #         for enemie_i, enemie  in enumerate(enemies):
    #             if min_enemie > enemies[enemie_i][1]:
    #         for e_laser_i, e_laser  in enumerate(e_lasers):
    #             if min_e_laser > e_lasers[e_laser_i][1]:
    #                 min_e_laser = e_lasers[e_laser_i][1]
    #         p_x = enemies[0][0] if min_enemie > min_e_laser else e_lasers[0][0]
    
    # k-tuika
    # if len(enemies) != 0 and len(e_lasers) != 0:
    if len(enemies) != 0:
        closest_enemy = enemies[0]
        # closest_laser = e_lasers[0]
        # if closest_enemy[1] > closest_laser[1]:
        p_x = closest_enemy[0]
        # else:
        #     p_x = closest_laser[0]
        if not is_shot:
    ...
    # if enemies:
    #     controler = True
    #     laser_position = p_x + p_width / 2 - p_laser_width / 2
    #     for sublist in p_lasers:
    #         if sublist[0] == laser_position:
    #             controler = False
    #             print('through')
    #             break
    #     if controler or not p_lasers:
    #         p_lasers.append(
    #             [p_x + p_width / 2 - p_laser_width / 2, p_y - p_laser_height]
    #         )
    #         p_laser_counter += 1
